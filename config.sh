...
asr_language_code=("fr-FR")

# ASR acoustic model architecture
# Supported values for the model architecture are:
# conformer (all languages except em-ea), parakeet (en-US, em-ea)
...
# parakeet: 0.6b (en-US only),0.6b_unified (en-US only), 1.1b (en-US only), 1.1b_unified_ml_cs (EMEA model only)
...
riva_ngc_org="nvidia"
riva_ngc_team="riva"
riva_ngc_image_version="2.16.0"
riva_ngc_model_version="2.16.0"
...
validate_asr_config() {
  if [[ ${asr_acoustic_model} == "parakeet" ]]; then
    if [[ ${asr_acoustic_model_variant} != "0.6b" && ${asr_acoustic_model_variant} != "0.6b_unified" && ${asr_acoustic_model_variant} != "1.1b" && ${asr_acoustic_model_variant} != "1.1b_unified_ml_cs" ]]; then
      echo "Parakeet model is available in 0.6b and 1.1b formats only."
      exit 1
    fi

    if [[ ${lang_code} != "en-US" && ${lang_code} != "em-ea" ]]; then
      echo "Parakeet is only available for language code en-US and em-ea"
      exit 1
    fi
    ...
    if [[ ${asr_acoustic_model_variant} == "1.1b_unified_ml_cs" && ${lang_code} != "em-ea" ]]; then
      echo " Parakeet unified and code switched model is only available for EMEA region. Please use em-ea as language code."
      exit 1
    fi
...
for lang_code in ${asr_language_code[@]}; do
    ...
    if [[ ${use_asr_greedy_decoder} = true ||
          ( ${lang_code} == "em-ea" && ${asr_acoustic_model_variant} == "1.1b_unified_ml_cs" && ${asr_acoustic_model} == "parakeet" ) ]]; then
      decoder="_gre"
    fi
    ...
    ### Punctuation model
    if [[ ${modified_asr_acoustic_model_variant} != *"_unified"* ]]; then
  ...
  if [[ $riva_target_gpu_family == "tegra" ]]; then
    if [[ ${lang_code} == "en-US" ]]; then
      ...
      if [[ ${lang_code} != "de-DE" ]]; then
        models_tts+=(
            "${riva_ngc_org}/${riva_ngc_team}/models_tts_magpie_multilingual_ipa:2.19.0-${riva_target_gpu_family}-${riva_tegra_platform}"
        )
      fi
      models_tts+=(
          "${riva_ngc_org}/${riva_ngc_team}/models_tts_magpie__multilingual_ipa:2.19.0-${riva_target_gpu_family}-${riva_tegra_platform}"
      )
    fi
  else
    if [[ ${lang_code} == "en-US" ]]; then
...
image_speech_api="riva-xtts:latest"
