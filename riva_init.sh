    ...
    if [ "$service_enabled_asr" = true ] || [ "$service_enabled_nlp" = true ] || [ "$service_enabled_tts" = true ] || [ "$service_enabled_nmt" = true ]; then
        ...
        if [[ $riva_target_gpu_family == "tegra" ]]; then
            ...
            docker exec riva-models-download bash -c \
              'cd /usr/local/bin; wget --no-verbose https://api.ngc.nvidia.com/v2/resources/nvidia/ngc-apps/ngc_cli/versions/3.50.0/files/ngccli_arm64.zip; \
...
if [[ $riva_target_gpu_family != "tegra" ]] || ([[ $riva_target_gpu_family == "tegra" ]] && [ -d "$riva_model_loc/rmir" ]); then
    ...
    docker run --init -it --rm --runtime=nvidia \
      -v $riva_model_loc:/data \
